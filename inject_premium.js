...
const newCSS = `
      ...
      .hero, .trust-strip, .stats-section, .testi-section { display: none !important; }
...
// 2. Inject CSS
const cssTarget = '.trust-inner.app-tab-section.active-tab { display: flex !important; }';
if (content.includes(cssTarget) && !content.includes('.mobile-app-header')) {
  content = content.replace(cssTarget, cssTarget + '\\n' + newCSS);
}

// 3. Inject HTML
const htmlTarget = '<!-- ══ CINEMATIC HERO ═════════════════════════════════════════ -->';
if (content.includes(htmlTarget) && !content.includes('<div class="mobile-app-header')) {
  content = content.replace(htmlTarget, htmlTarget + '\\n' + newHTML);
}
